...
export class RenderPipeline {
	...
	public resize( layerInfo: ORE.LayerInfo ) {
		...
		this.renderTargets.rt1.setSize( layerInfo.size.canvasPixelSize.x, layerInfo.size.canvasPixelSize.y );
		this.renderTargets.rt2.setSize( layerInfo.size.canvasPixelSize.x, layerInfo.size.canvasPixelSize.y );
		this.renderTargets.rt3.setSize( layerInfo.size.canvasPixelSize.x, layerInfo.size.canvasPixelSize.y );

		this.smaaCommonUni.SMAA_RT_METRICS.value.set(
			1 / ( layerInfo.size.canvasPixelSize.x ),
			1 / ( layerInfo.size.canvasPixelSize.y ),
			layerInfo.size.canvasPixelSize.x,
			layerInfo.size.canvasPixelSize.y
		);

		for ( let i = 0; i < this.bloomRenderCount; i ++ ) {

			let size = layerInfo.size.canvasPixelSize.clone().multiplyScalar( this.bloomResolutionRatio );
	...
	private renderMainVisual( scene: THREE.Scene, camera: THREE.Camera ) {
		...
		this.compositePP.render( compositeInputRenderTargets, this.renderTargets.rt3 );
